...
class OVContrlNetStableDiffusionPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        scheduler,
        core: ov.Core,
        controlnet: Union[List[ov.Model], Tuple[ov.Model]],
    ...
    def load_models(self, core: ov.Core, device: str, controlnet:Union[List[ov.Model], Tuple[ov.Model]], text_encoder: ov.Model, unet: ov.Model, vae_decoder: ov.Model):
        """
        ...
        compiled_controlnet = [core.compile_model(a_controlnet, device) for a_controlnet in controlnet]
        self.register_to_config(controlnet=compiled_controlnet)
    ...
    def __call__(
        ...
        image_lst: Union[Tuple[Image.Image], List[Image.Image]],
        controlnet_conditioning_scale: Union[Tuple[float], List[float]],
        ...
        pad_lst = []
        new_image_lst = []
        for an_image in image_lst:
            orig_width, orig_height = an_image.size
            image, pad = preprocess(an_image)
            pad_lst.append(pad)
            height, width = image.shape[-2:]
            if do_classifier_free_guidance:
                image = np.concatenate(([image] * 2))
            new_image_lst.append(image)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for net_idx in range(len(self.controlnet)):
                    a_net = self.controlnet[net_idx]
                    result = a_net([latent_model_input, t, text_embeddings, new_image_lst[net_idx]])
                    if net_idx == 0:
                        down_and_mid_blok_samples = [sample * controlnet_conditioning_scale[net_idx] for _, sample in result.items()]
                    else:
                        temp_samples = [sample * controlnet_conditioning_scale[net_idx] for _, sample in result.items()]
                        for sample_idx in range(len(down_and_mid_blok_samples)):
                            down_and_mid_blok_samples[sample_idx] += temp_samples[sample_idx]
